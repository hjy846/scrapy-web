...
import scrapy
from crawler.items import ResidenceRentItem, ResidenceRentDetailItem, ResidenceRentImageItem
import sys
import time
import urlparse
import os
...
class ResidenceSpider(scrapy.Spider): 
    ...
    def __init__(self, crawl_date = None, region = None, *args, **kwargs):
        ...
        #房产列表网页保存路径
        self.list_output_path = './output/list_rent/%s/%s/%s/' % (self.crawl_date, self.region, self.now)
        ...
        #房产详情页面保存路径
        self.detail_output_path = './output/detail_rent/%s/%s/%s/' % (self.crawl_date, self.region, self.now)
        ...
        #房产图片保存路径
        self.image_output_path = './output/image_rent/'
    ...
    def parse_list(self, response):
        ...
        for sel in response.xpath('//div[@class="result-list"]'):
            try:
                ...
                item['rent_price'] = sel.xpath('div[@style="float:right"]/div[@class="result-list-r"]/div[@class="result-list-r-price red"]/text()').re('(\d+)')[0]
                item['rent_price'] = int(item['rent_price'])
    ...
    def parse_detail(self, response):    
        ...
        for sel in response.xpath('//div[@class="module1 left"]'):
            try:
                ...
                item['rent_price'] = sel.xpath('div[@class="view-price"]/div[@class="view-price-div view-price-yellow"]/div/text()').extract()[0].strip('$').strip('-')
                item['rent_price'] = int(item['rent_price'])
                ...
                image_item = ResidenceRentImageItem()
