...
def find_new_residences():
    ...
    for r in res:
        ...
        #替换image url
        #print _id
        save_record = {'info':r, 'update_time':datetime.now(), 'price_history.%s' % r['update_time']:r['rent_price']}
        #print save_record
...
RAW_DB = settings['MONGODB_DB_PARKING_RENT_RAW']
CONNECTION = pymongo.MongoClient(SERVER, PORT)
RAW_DB = CONNECTION[RAW_DB] 
DB = CONNECTION[DB]  
RAW_COLLECTION = RAW_DB[PARAMS['crawl_date']]
IMAGE_COLLECTION = RAW_DB[settings['MONGODB_COLLECTION_PARKING_IMAGE']]

COLLECTION = DB[settings['MONGODB_COLLECTION_ALL_RENT_PARKINGS']]
COLLECTION_NEW_ADD = DB[settings['MONGODB_COLLECTION_NEW_ADD_RENT_PARKING']]
COLLECTION_RESIDENCE_NUM_BY_DAY = DB[settings['MONGODB_COLLECTION_RENT_PARKING_NUM_BY_DAY']]

if __name__ == '__main__':
    #PARAMS['crawl_date'] = "2017-03-08"
